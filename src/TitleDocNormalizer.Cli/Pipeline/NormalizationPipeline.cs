    ...
    OutputWriter outputWriter)
{
    public async Task<PipelineResult> RunAsync(CliOptions options, CancellationToken cancellationToken = default)
    {
        ...
        var manifest = new NormalizationManifest(
            ...
            Notes: "Optimized title-document normalization with page classification, section extraction, and field-aware routing.");

        await outputWriter.WriteAsync(options.OutputFolder, rawPages, normalizedPages, classifiedPages, sections, routes, manifest, cancellationToken);
        ...
        if (options.GeneratePrompt)
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(promptDir, "field_extraction_prompt.md"), prompt, cancellationToken);
        }
